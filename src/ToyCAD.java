	...
	private static boolean executeCommand(Map<String,Object> parsedCommand, List<IShape> shapes ) {
		switch ((String)parsedCommand.get("command")) {
			...
			case "COPY": //specifically debug this
				shapes.get((int) parsedCommand.get("ID")).copy().move((double) parsedCommand.get("moveX"),(double) parsedCommand.get("moveY"));
				return false;
			case "AREA": 
				printTotalColored(shapes,(IShape.Color) parsedCommand.get("color"), Method.AREA);
				return false;
			case "COLOR":
				shapes.get((int) parsedCommand.get("ID")).setColor((IShape.Color) parsedCommand.get("color"));
				return false;
			case "CIRCUMFERENCE":
	...
	private static Map<String, Object> parseLine(String line) {
		...
		switch (splittedLine.get(0)) {
			...
			case ("DELETE"):
				...
				break;
			case ("MOVE"): // case move and copy are the same, it will add the same arguments to the parsedCommand 
			case ("COPY"):
				parsedCommand.put("ID",    Integer.valueOf(splittedLine.get(1)));
